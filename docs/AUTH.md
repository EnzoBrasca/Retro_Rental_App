...
> **Por qué el registro pide dirección y teléfono.** La entidad `Persona` tiene
> `@Embedded Direccion` y `Telefono` con **todas las columnas `NOT NULL`**. Si el
> registro no las enviara, el `INSERT` en `personas` fallaría. Se decidió pedirlas
> en el alta (en vez de hacerlas opcionales) para no migrar el esquema. El front
> las manda como objetos anidados y el backend las valida con `@Valid`.
...
## 9. Manejo de errores (RESUELTO — ver `docs/ERRORS.md`)

> Versiones anteriores de esta guía advertían que el backend devolvía HTTP 500
> sin mensaje ante credenciales inválidas. **Eso ya está resuelto.**

El backend tiene un `@RestControllerAdvice` (`GlobalExceptionHandler`) que
traduce las excepciones a respuestas JSON `{ status, message }`:

| Situación | Status | Mensaje |
|-----------|--------|---------|
| Credenciales inválidas (login) | **401** | `Email o contraseña incorrectos` |
| Email/documento ya registrado | **409** | `Ya existe un usuario con ese email` / `...documento` |
| Validación `@Valid` (campo faltante/mal) | **400** | mensajes de cada campo, unidos por `; ` |

El front ya lee `error.message`, así que muestra el texto real. El detalle
completo del modelo de errores (incluida la causa raíz del **403 espurio** que
enmascaraba estos errores) está en `docs/ERRORS.md`.
